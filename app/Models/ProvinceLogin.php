...
class ProvinceLogin extends Authenticatable implements JWTSubject
{
    use Notifiable ;

    protected $table = 'p_login';
    ...
    {
        $student_job_number = $request['username'];
        try{
            $count = self::select('username')
                ->where('username',$student_job_number)
                ->count();
            //echo "该账号存在个数：".$count;
            //echo "\n";
            return $count;
