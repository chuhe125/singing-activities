...
class UsersController extends Controller
{
    ...
    {
        $count = UsersLogin::checknumber($registeredRequest);   //检测账号密码是否存在
        if($count == 0)
        {
            $student_id = UsersLogin::createUser(self::userHandle($registeredRequest));
            $created_at = now();
            $updated_at = now();
            $res = DB::table('jurisdiction')->insert([
                'u_login_id'=>$student_id,
    ...
    public function login(Request $request)
    {
        ...
        //方式二
        $id = DB::table('u_login')->where('shool_name',$credentials['shool_name'])->value('id');

        $state = DB::table('jurisdiction')->where('u_login_id',$id)->value('state');
        if($state==0)
        $token = Auth::guard('api')->attempt($credentials);   //获取token
        else
            return  json_fail('暂无权限',null, 100 ) ;
//        if(!$token){
...
//        return self::respondWithToken($token, '登录成功!');   //可选择返回方式
    ...
    {
        return ['shool_name' => $request['shool_name'], 'password' => $request['password']];
    }
    ...
    protected function userHandle($request)   //对密码进行哈希256加密
    {
        ...
        $registeredInfo['shool_name'] = $registeredInfo['shool_name'];
