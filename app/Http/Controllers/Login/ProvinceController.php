...
use App\Models\CityLogin;
use App\Models\ProvinceLogin;
...
class ProvinceController extends Controller
{
    ...
    {
        $count = ProvinceLogin::checknumber($registeredRequest);   //检测账号密码是否存在
        if($count == 0)
        {
            $student_id = ProvinceLogin::createUser(self::userHandle($registeredRequest));
            return  $student_id ?
    ...
    public function login(Request $request)
    {
        ...
        //方式二
        $token = Auth::guard('province')->attempt($credentials);   //获取token
//        if(!$token){
