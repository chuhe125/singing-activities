...
class CityController extends Controller
{
    ...
    {
        $count = CityLogin::checknumber($registeredRequest);   //检测账号密码是否存在
        if($count == 0)
        {
            $student_id = CityLogin::createUser(self::userHandle($registeredRequest));
            return  $student_id ?
    ...
    public function login(Request $request)
    {
        ...
        //方式二
        $token = Auth::guard('city')->attempt($credentials);   //获取token
//        if(!$token){
